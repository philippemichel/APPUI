---
title: "Rapport statistique V1.2"
cache: false
---
...
L'item `Fragilité` est très peu rempli ainsi que dans une moindre mesure `allergie`. les autres items comportant de nombreuses données manquantes sont celles commençant par `si\dots` ce qui est normal ou ceux concernant les exemnnes etc. après la sortie. 
...
```{r}
#| label: tbl-diag
#| tbl-cap: Diagnostics retenus & conformité aux recommandations

tt |> 
  dplyr::select(diagnostic_retenu, pac_recos) |> 
    tbl_summary(
  by = pac_recos,
    missing = "no",
  percent = "row"
  ...
  bold_labels() |>
  modify_header(label ~ " ") |>
  pexptabph(exp = expx, nomfich = classeur, nomsheet = "diag", lg = TRUE)

```

```{r}
#| label: fig-diag
#| fig-cap:  Diagnostics retenus & conformité aux recommandations

zz <- tt |> 
  group_by(pac_recos) |> 
 count(diagnostic_retenu) |> 
  dplyr::filter(pac_recos == "Oui")
...
zz |> 
  mutate(pc = 100*n/nn) |> 
    ggplot() + 
  aes(x = fct_reorder(diagnostic_retenu,pc), y = pc) +
 geom_segment(aes(x = fct_reorder(diagnostic_retenu,pc),xend = fct_reorder(diagnostic_retenu,pc),y = 0,, yend = pc)) +
  geom_point(size = 3) +
  coord_flip() +
  labs(title = "Conformité aux recommandations\n selon le diagnostic (%)",
      subtitle = "",
      y = "%",
      caption = "",
      fill = "") +
...
glm(pac_recos ~ sexe + age + temperature +immunodepression , data = tt, family = "binomial") |> 
...
L'analyse en régression confirme que la prise en charge des patients de plus de 75 ans est environ trois fois moins souvent conforme aux recommandations que pour les patients les plus jeunes.

```{r}
#| label: fig-ageprinc
#| fig-cap: Conformité de la prise en charge conforme selon l'âge du patient

barouiph(dfx = tt, testx = age, varx = pac_recos, valx = "Oui",
  titre = "Conformité de la\n prise en charge selon l'âge du patient")
```
...
tt |>
  ...
  add_ci() |>
  modify_header(label ~ " ") |>
  pexptabph(exp = expx, nomfich = classeur, nomsheet = "crits1", lg = TRUE)
```
...
### Examens demandés aux urgences & pertinence
...
tt |>
  ...
  bold_labels() |>
  modify_header(label ~ " ") |>
  gexptabph(exp = expx, nomfich = classeur, nomsheet = "examcomp1", lg = FALSE)
```
...
tt |>
  ...
  bold_labels() |>
  modify_header(label ~ " ") |>
  pexptabph(exp = expx, nomfich = classeur, nomsheet = "examcomp2", lg = FALSE)
```
...
tt |>
  ...
  bold_labels() |>
  modify_header(label ~ " ") |>
  gexptabph(exp = expx, nomfich = classeur, nomsheet = "examcomps1", lg = TRUE)
...
tt |>
  ...
  mutate(specialite = fct_infreq(specialite)) |> 
  tbl_summary(
    missing = "no",
    statistic = stt,
    value = ouinon
...
Les demandes d'avis spécialisés représentent `r aftrans(tt$avis_specialise)` (Pourcentage avec son intervalle de confiance à 95 %).
...
tt |>
  ...
  bold_labels() |>
  modify_header(label ~ " ") |>
  pexptabph(exp = expx, nomfich = classeur, nomsheet = "germes1", lg = TRUE)
```

```{r}
#| label: fig-microb
#| fig-cap: "Germes retrouvés (ECBU)"

st <- paste0("Germes retrouvés aux urgences (N = ", length(na.omit(tt$germe)), ")")

tt |> 
  drop_na(germe) |>
  group_by(germe) |> 
  summarise(n = n()) |> 
  mutate(gram = ifelse(germe %in% c("Corynebacterium aurimucosum", "Enterococcus fæcalis", "Enterococcus hiræ", "Staphylococcus aureus", "Staphylococcus saprophyticus", "Streptococcus aggalactiæ" , "Streptococcus anginosus") , "Gram +", "Gram -")) |> 
  ggplot() + 
  aes(x = fct_reorder(germe,n), y = n, color = gram) +
 geom_segment(aes(x = fct_reorder(germe,n),xend = fct_reorder(germe,n),y = 0,, yend = n)) +
  geom_point(size = 3) +
  coord_flip() +
  labs(title = "Germes retrouvés",
      subtitle = "",
      y = "n",
      caption = st,
      fill = "") +
...
zz <- tt |> 
  dplyr::select(-c(id, fc, traitement_propose,specialite,germe_identifie,microbio_dispo,exam_comp,germe)) |> 
  dplyr::select(!starts_with("si_")) |> 
...
Seules les variables les plus pertinentes sont présentées. L'abcisse présente une orientation femme/homme. L'ordonné semble être orientée plus selon la complexité du cas & ce plutôt chez l'homme (les cas chez les femmes semblent être globalement plus simples). 
...
#| #| fig-asp: .8
#| eval: false
res <- hcut(zzc, k = 2, stand = TRUE)
# Visualize
fviz_dend(res, rect = TRUE, cex = 0.5)
```

La classification non supervisée ne donne pas de résultat intéressant.
...
le \gls{alpha} retenu était de 0,05 & la \gls{puissance} de 0,8. Les analyses ont été réalisées avec le logiciel \textsc{r} version 4.3.0 (R Core Team, 2025).

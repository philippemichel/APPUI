...
La $PA_{m}$ a été calculée selon l'approximation usuelle : $PA_{m} = \frac{PA_{syst} + 2 PA_{diast}}{3}$.
...
Les variables constantes seront supprimées dans le sreste du travail, il en reste `r ncol(tt)` variables.
...
glm(pac_recos ~ sexe + age +ta + fc, data = tt, family = "binomial") |> 
  forest_model()
```
...
glm(conforme_spilf ~ sexe + age +ta + fc, data = tt, family = "binomial") |> 
  forest_model()
```
## Objectif II
...
```{r}
#| label: tbl-examcomp2
#| tbl-cap: Pertinence des examens demandés
